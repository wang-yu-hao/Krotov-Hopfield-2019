...
output = Sequential()
output.add(Dense(2, activation = 'tanh', input_dim = hid))
output.compile(optimizer = opt, loss = customloss, metrics = ['accuracy'])
label_train = keras.utils.to_categorical(label_train, num_classes = 2)
label_train_rand = keras.utils.to_categorical(label_train_rand, num_classes = 2)
label_test = keras.utils.to_categorical(label_test, num_classes = 2)
